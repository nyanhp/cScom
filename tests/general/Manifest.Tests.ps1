Describe "Validating the module manifest" {
	$moduleRoot = (Resolve-Path "$global:testroot\..\cScom").Path
	$manifest = ((Get-Content "$moduleRoot\cScom.psd1") -join "`n") | Invoke-Expression
	Context "Basic resources validation" {
		$resources = Get-ChildItem "$moduleRoot\resources" -Recurse -File | Where-Object Name -like "*.ps1"
		$files = Get-ChildItem "$moduleRoot\functions" -Recurse -File | Where-Object Name -like "*.ps1"
		It "Exports all resources in the public folder" -TestCases @{ files = $resources; manifest = $manifest } {
			
			$functions = (Compare-Object -ReferenceObject ($files.BaseName -replace 'JHP_') -DifferenceObject $manifest.DscResourcesToExport | Where-Object SideIndicator -Like '<=').InputObject
			$functions | Should -BeNullOrEmpty
		}
		It "Exports all functions in the public folder" -TestCases @{ files = $files; manifest = $manifest } -Skip:$($files.Count -eq 0 -and $manifest.FunctionsToExport.Count -eq 0) {
		...
		It "Exports no function that isn't also present in the public folder" -TestCases @{ files = $files; manifest = $manifest } -Skip:$($files.Count -eq 0 -and $manifest.FunctionsToExport.Count -eq 0) {
			$functions = (Compare-Object -ReferenceObject $files.BaseName -DifferenceObject $manifest.FunctionsToExport | Where-Object SideIndicator -Like '=>').InputObject
			$functions | Should -BeNullOrEmpty
		}
